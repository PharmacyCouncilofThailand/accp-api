  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      if (status === 401 && retryOnAuth) {
        cachedToken = null;
        tokenExpiry = 0;
        return sendNipaMailEmail(recipient, subject, html, false);
      }

      // Handle other errors
      throw new Error(
        `NipaMail error (${status}): ${errorType || error.response.data?.message || "Unknown error"}`
      );
    }
    throw error;
  }
...
export async function sendAbstractSubmissionEmail(
  ...
  try {
    await sendNipaMailEmail(email, "Abstract Submission Confirmation - ACCP 2026", html);
...
export async function sendCoAuthorNotificationEmail(
  ...
  try {
    await sendNipaMailEmail(email, "You've been added as Co-Author - ACCP 2026 Abstract", html);
...
export async function sendPendingApprovalEmail(
  ...
  try {
    await sendNipaMailEmail(email, "Registration Received - Pending Verification", html);
...
export async function sendVerificationApprovedEmail(
  ...
  try {
    await sendNipaMailEmail(email, "Account Approved - ACCP Conference 2026", html);
...
export async function sendPasswordResetEmail(
  ...
  try {
    await sendNipaMailEmail(email, "Reset Your Password - ACCP Conference 2026", html);
